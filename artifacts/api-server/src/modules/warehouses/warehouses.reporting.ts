...
export function buildInventoryReport(input: {
  ...
  const currentStock = getCount(FABRIC_ROLL_WORKFLOW_STATUS.inStock) + getCount(FABRIC_ROLL_WORKFLOW_STATUS.reserved);
  ...
  return {
    ...
    reserved: getCount(FABRIC_ROLL_WORKFLOW_STATUS.reserved),
