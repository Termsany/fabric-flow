    ...
    <html lang="${locale.startsWith("ar") ? "ar" : "en"}" dir="${locale.startsWith("ar") ? "rtl" : "ltr"}">
      ...
      <body>
        <div class="card">
          ...
          <div class="row"><span>${labels.invoiceNumber}</span><span class="strong">${invoice.invoiceNumber}</span></div>
          <div class="row"><span>${labels.amount}</span><span class="strong">${invoice.amount} ${invoice.currency}</span></div>
          <div class="row"><span>${labels.status}</span><span>${invoice.status}</span></div>
          <div class="row"><span>${labels.issuedAt}</span><span>${issuedAt}</span></div>
          <div class="row"><span>${labels.dueAt}</span><span>${dueAt}</span></div>
          <div class="row"><span>${labels.paidAt}</span><span>${paidAt}</span></div>
          <div class="row"><span>${labels.notes}</span><span>${invoice.notes ?? "-"}</span></div>
        </div>
      </body>
    </html>
  `);
          ...
          <div className="mb-6 grid gap-6 lg:grid-cols-[1.1fr,1.4fr]">
            <section className="rounded-3xl bg-[linear-gradient(135deg,#0f172a,#1e293b_55%,#0f766e)] p-6 text-white shadow-xl">
              <div className="mb-5 flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${getSubscriptionStatusTone(data.statusSummary)}`}>
                      {getSubscriptionStatusLabel(data.statusSummary, t)}
                    </span>
                    <span className="text-sm text-slate-200">{getSubscriptionStatusMessage(data.statusSummary, t)}</span>
                  </div>
                </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-2xl bg-white/10 p-4">
                  <div className="text-xs text-slate-200">{getSubscriptionNextEventLabel(data.statusSummary, t) || t.trialEndsAt}</div>
                  <div className="mt-1 text-sm font-semibold">{formatDate(data.statusSummary.nextRelevantAt || data.trialEndsAt, locale)}</div>
                </div>
                <div className="rounded-2xl bg-white/10 p-4">
                ...
                <div className="rounded-2xl bg-white/10 p-4">
                  <div className="text-xs text-slate-200">{t.accessStatus}</div>
                  <div className="mt-1 text-sm font-semibold">{data.statusSummary.hasAccess ? t.accessActive : t.accessRestricted}</div>
                </div>
                <div className="rounded-2xl bg-white/10 p-4">
                  <div className="text-xs text-slate-200">{t.lastInvoiceStatus}</div>
                  <div className="mt-1 text-sm font-semibold">{data.statusSummary.lastInvoiceStatus || "-"}</div>
                </div>
                <div className="rounded-2xl bg-white/10 p-4">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {data.statusSummary.needsAttention && (
                <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                  {getSubscriptionStatusMessage(data.statusSummary, t)}
                </div>
              )}
              {rejectedPayment && (
              ...
              {data.statusSummary.isEndingSoon && (
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-[1fr,1fr]">
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {arePaymentsLoading ? (
                ...
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
                  {t.subscriptionPaymentsEmptyState}
                </div>
              ) : (
                <div className="space-y-3">
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {areInvoicesLoading ? (
                ...
                <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-800">
                  {invoicesError instanceof Error ? invoicesError.message : t.billingActivityLoadError}
                </div>
              ) : invoices.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">
                  {t.subscriptionInvoicesEmptyState}
                </div>
              ) : (
                <div className="space-y-3">
                    ...
                    <div key={invoice.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                      ...
                      <div className="mt-3 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-600">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            printInvoice(invoice, t.companyBillingDocument, locale, t.invoiceHistory, {
                              invoiceNumber: t.invoiceNumberLabel,
                              amount: t.amountLabel,
                              status: t.statusLabel,
                              issuedAt: t.issuedAtLabel,
                              dueAt: t.dueAtLabel,
                              paidAt: t.paidAtLabel,
                              notes: t.notesLabel,
                            })
                          }
